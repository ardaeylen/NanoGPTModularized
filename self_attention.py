...
class SelfAttention(nn.Module):
    def __init__(self, context_length, embedding_size, query_dim, value_dim, num_heads):
        ...
        self.query_layer = nn.Linear(embedding_size, self.query_dim)
        self.key_layer = nn.Linear(embedding_size, self.query_dim)
        self.value_layer = nn.Linear(embedding_size, self.value_dim)
    ...
    def forward(self, x): # Assume that x is (B, T (Context Length), Embedding Dim)
        ...
        attn_wei = ((queries @ keys.transpose(2, 3)) / self.query_dim ** (1/2)) @ values
        attn_wei = attn_wei.view(B, T, -1)
        return attn_wei
'''
...
class SelfAttentionKarpathy(nn.Module):
    def __init__(self, embedding_size, head_size, context_length, dropout_rate):
    ...
    def forward(self, x): # Assume that x is (B, T (Context Length), Embedding Dim)
        ...
        affinities = affinities.masked_fill(self.tril == 0, float('-inf')) / (1/self.head_size**0.5)
...
if __name__ == "__main__":
    attn = SelfAttentionKarpathy(256, 64, 16)
    x = torch.randn((1, 64, 256))
    output = attn(x)
    print(output)
'''
if __name__ == "__main__":
    attn = SelfAttention(256, 64, 16)
    x = torch.randn((1, 64, 256))
    output = attn(x)
    print(output)
'''